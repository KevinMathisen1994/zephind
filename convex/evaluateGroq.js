...
export const evaluateWithGroq = action({
  args: {
...
- 最後に「総評: まとめ」のように短く締めくくること。`;
    ...
    if (placesKey && property.address) {
      try {
        ...
        console.error("[Groq Places] Error:", e);
      }
    }
    ...
    if (reinfolibKey && property.ward) {
      try {
        ...
        if (cityCode) {
          console.log(`[Groq MLIT] Fetching for ${property.ward}...`);
        ...
        console.error("[Groq MLIT] Error:", e);
      }
    }
    ...
    try {
      ...
      console.log(`[Groq] Score: ${score}/100 for ${property.address || property.ward || "?"} (model: ${modelName})`);
      ...
      if (property.matchId) {
        try {
          ...
          console.error("[Groq] DB save failed:", dbErr);
        }
      }
